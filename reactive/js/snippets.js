...
function getJSON(ary){
  return Rx.Observable.from(ary)
    .map( (str) => { let parsedJSON = JSON.parse(str); return parsedJSON; }  )
}
/**
* snippets
*/
...
function snippet5() {
  ...
  test.subscribe(
    onNext = (data) => { console.log('Results ' + data);},
    onError = (err) => { console.log('Error ' + err);},
    onCompleted = () => { console.log('Completed');}
  );
...
function snippet6() {
  Rx.DOM.get('http://jsonplaceholder.typicode.com/posts')
    .subscribe(
      onNext = (data) => { console.log('Results ' + data.response);},
      onError = (err) => { console.log('Error ' + err);}
    )
}
...
function snippet7() {
  ...
  let allMovesOnTheLeft = allMoves.subscribe(
    onNext = (x) => { if (x.clientX <= window.innerWidth / 2 ) {
  ...
  allMovesOnTheLeft.subscribe( (x) => { console.log('2 - Move on the left: ' , x.clientX, x.clientY); }  );
  allMovesOnTheRight.subscribe( (x) => { console.log('2 - Move on the right: ' , x.clientX, x.clientY); }  );
...
function snippet8() {
  ...
  setTimeout(() => {console.log('Cancelling subscription');subscription.dispose();},2000);
}
...
function snippet9() {
  ...
  let avg = src.reduce( (acc, current) =>
  ...
  avg = src2.scan( (acc, current) =>
     { return { sum: acc.sum + current, count: acc.count + 1} }
  ,  {sum:0, count:0})
  .map( (x) => x.sum / x.count);
  let subscription = avg.subscribe( (avg) => console.log('avg from scan: ' + avg))

  setTimeout( () => {console.log('Cancelling avg subscription');subscription.dispose()},3000)
...
function snippet10() {
  ...
  let caught = getJSON([ '{"a":1,"b":2}', '{"success: true}' ])
  .catch(Rx.Observable.return( {error:'error while parsing JSON'} ));

  caught.subscribe(
    (json) => { console.log('2-Parsed JSON: ', json);},
...
function execSnippet(){
  ...
  switch (snippet) {
    ...
    case '5':  snippet5();  break;
    case '6':  snippet6();  break;
    case '7':  snippet7();  break;
    case '8':  snippet8();  break;
    case '9':  snippet9();  break;
    case '10': snippet10(); break;
    case '11': snippet11(); break;
    default:   console.log('Choose a RxJS snippet to run and see the code');
  }
}
